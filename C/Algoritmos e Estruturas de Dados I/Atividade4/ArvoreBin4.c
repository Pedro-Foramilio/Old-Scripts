...
int main()
{
	...
	while (scanf("%s", comando) != EOF)
	{
		...
		else
		{
			...
			{
				inserir(arvore, criaNo(calculaChave(palavra), palavra));
			}
			else if (strcmp(comando, "delete") == 0)
			{
				no = remover(arvore, calculaChave(palavra));
				if (no != NULL)
				{
                    free(no->palavra);
					free(no);
...
No * criaNo(int chave, char *palavra)
{
    No *no = (No *) malloc(sizeof(No));
    if (no != NULL)
    {
        no->chave = chave;
        no->palavra = (char *) malloc(sizeof(char) * 100);
